...
######Notice: the peaks may overlap with multiple exons
exon_gtf <- gtf%>%dplyr::filter(V3=="CDS")

##transform gtf file into bed file format
...
#out.dir <- "/Users/sidaye/Documents/R/Cut_Run_Manish/Manish_lac/Output/gene_peak_assignment_broadPeak2/unfiltered/8mM/"
out.dir <- "/Users/sidaye/Documents/R/Cut_Run_Manish/Manish_lac/Output/gene_peak_assignment_broadPeak2/unfiltered/UNT/"
...
#####broad peak is hard to add gene annotations once cover more than 1 gene#########
for (i in 1:nrow(sample_table)){
  ...
  #V32 is the TSS loci#V4 is peakid chr:start-end, V29 is geneID
  ...
  genic.peaks2_final <- genic.peaks2%>%transmute(Chr = V1, str = V2, end = V3, peak_id = V4, signalValue1=V11, pValue1=V12, qValue1=V13, peak_id1=V24,
              signalValue2=V20, pValue2=V21, qValue2=V22, peak_id2=V25,GeneID = covered_genes, overlapped=V3-V2,num=num,Category = 'Covered more than one gene')
  ...
  nongenic.peaks <- peaks.all.sort %>% dplyr::filter(!(peak_id %in% (unique(genic.peaks_m$V4))))
  ...
  if (nrow(nongenic.promoter.peaks) == 0) {
    ...
    intergenic.peaks <- nongenic.peaks %>% dplyr::filter(!(peak_id %in% (unique(nongenic.promoter.peaks$V4))))
  }
  nongenic.promoter.peaks_final <- nongenic.promoter.peaks%>%transmute(Chr = V1, str = V2, end = V3, peak_id = V4, signalValue1=V11, pValue1=V12, qValue1=V13, peak_id1=V24,
                                                                       signalValue2=V20, pValue2=V21, qValue2=V22, peak_id2=V25,GeneID = V29, overlapped=V3-V2,num=1,Category = Category)
  #####To calculate the percentage of overlapped regions over intersected regions#####
  nongenic.promoter.peaks_final$intersected_peak_width <-nongenic.promoter.peaks_final$end-nongenic.promoter.peaks_final$str
  nongenic.promoter.peaks_final$overlapped_percentage <- nongenic.promoter.peaks_final$overlapped/nongenic.promoter.peaks_final$intersected_peak_width
  nongenic.promoter.peaks_final <- nongenic.promoter.peaks_final%>%group_by(peak_id)%>% mutate(unique_assignment = Category[which.max(overlapped_percentage)]) %>%ungroup()
  ...
  if (nrow(intergenic.peaks.genes.dist)==0){
    ...
    gene.id <- unlist(lapply(strsplit(intergenic.peaks.genes.dist$V34, split = ''), function(x){
      n = length(x)
    ...
    ## V36 <= 0 means the peak is at upstream of TSS, V36 >0 means peak is downstream of TES
    
    ##V36 shows the distance to nearest gene
    #####Those intergenic peaks are too far away from the genic regions 2000bp will not be assigned a gene
    intergenic.peaks.genes.dist2<- intergenic.peaks.genes.dist %>% dplyr::mutate(V28=ifelse(abs(V36) < 2000,V28,NA))
    ...
    intergenic.peaks_final <- intergenic.peaks.genes.dist2 %>% 
      transmute(Chr = V1, str = V2, end = V3, peak_id = V4, signalValue1=V11, pValue1=V12, qValue1=V13, peak_id1=V24,
                signalValue2=V20, pValue2=V21, qValue2=V22, peak_id2=V25,GeneID = V28, overlapped=V3-V2,num=num,Category = 'Intergenic')
    
    intergenic.peaks_final$intersected_peak_width <-intergenic.peaks_final$end-intergenic.peaks_final$str
    intergenic.peaks_final$overlapped_percentage <- intergenic.peaks_final$overlapped/intergenic.peaks_final$intersected_peak_width
  ...
  if(nrow(genic.peaks1)==0){
    ...
    genic.peaks1_final2 <- genic.peaks1_final %>%
      transmute(Chr = V1, str = V2, end = V3, peak_id = V4, signalValue1=V11, pValue1=V12, qValue1=V13, peak_id1=V24,
                signalValue2=V20, pValue2=V21, qValue2=V22, peak_id2=V25,GeneID = V29, overlapped=V42,num=1,Category = Category)
