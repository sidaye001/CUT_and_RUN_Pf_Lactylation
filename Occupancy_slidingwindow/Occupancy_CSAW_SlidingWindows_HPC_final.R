...
odd_vec <- seq(1, 48, by = 2)

output <- "/home/sida.ye001/cut_and_run/Output/Occupancy_slidewindow/peak_results/"
output_bed <- "/home/sida.ye001/cut_and_run/Output/Occupancy_slidewindow/bed/"
for(i in odd_vec){
  #vec <- c(i,i+1,i+24,i+25)
  condition <- bamfiles_table$Condition[i]
  stage <- bamfiles_table$Tissue[i]
  antibody <- bamfiles_table$Factor[i]
  bam_files <- c(trimws(bamfiles_table$bamReads[i]), trimws(bamfiles_table$bamReads[i+1]), trimws(bamfiles_table$bamControl[i]), trimws(bamfiles_table$bamControl[i+1]))
  ...
  # Experimental design: High means samples, Low means IgG controls
  ...
  sample_name <- paste0(antibody,"_",stage,"_",condition)
  ...
  write.xlsx(all.results,paste0(output,sample_name,"_Occupancy_slidingwindow_peak_result.xlsx"))
  ...
  options(scipen=999)
  write.table(sig_bed,paste0(output_bed, sample_name,"_Occupancy_slidingwindow_peaks.bed" ),sep="\t",col.names=FALSE,quote=FALSE,row.names=FALSE)
}
